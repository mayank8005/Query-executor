...
const QueryResult = (props) => {
  ...
  const cols = useMemo(() => {
      ...
      return props.queryResult[0].map((header) => ({label: header, dataKey: header, width: 200}))
  }, [props.queryResult]);

  const rows = useMemo(() => {
    if(!props.queryResult || !props.queryResult.length >= 2) return [];
    console.log(props.queryResult.slice(1).map(row => (row.reduce((acc, curr, idx) => ({...acc, [props.queryResult[0][idx]]: curr}), {}))))
    return props.queryResult.slice(1).map(row => (row.reduce((acc, curr, idx) => ({...acc, [props.queryResult[0][idx]]: curr}), {})))
    }, [props.queryResult]);

  const cellRenderer = ({ cellData }) => (
    <div>
       <Tooltip title={cellData}>
        <div className="text-black whitespace-nowrap overflow-hidden font-bold text-sm text-ellipsis w-11/12">{cellData}</div>
       </Tooltip>
    </div>
  );
